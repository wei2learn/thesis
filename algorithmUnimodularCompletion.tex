...
\end{comment}
\begin{algorithm}[t]
\caption{$\unimodularCompletion(\mathbf{F},\vec{s})$}
\label{alg:unimodularCompletion}

\begin{algorithmic}
[1]\REQUIRE{$\mathbf{F}\in\mathbb{K}\left[x\right]^{m\times n}$ with $I_{m}$
as a GCD; $\vec{s}$ is initially set to the column degrees of $\mathbf{F}$.
